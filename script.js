  ...
  if (dotsWrap && cards.length > 0) {
    ...
    track.addEventListener('scroll', () => {
      ...
      scrollTimeout = setTimeout(() => {
        ...
        // Verifica se a rolagem chegou ao limite direito (com margem de 5px para arredondamentos)
        const isAtEnd = Math.ceil(track.scrollLeft + track.clientWidth) >= track.scrollWidth - 5;

        if (isAtEnd) {
          // Força a última bolinha a ficar ativa no final do scroll
          closest = cards.length - 1;
        } else {
          // Calcula o card mais próximo da borda esquerda
...
if (autoTrack && autoTrack.children.length > 0) {
